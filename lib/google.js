...
// Lê os valores de uma planilha Google (conta de serviço precisa ter acesso de leitura)
async function lerPlanilha(spreadsheetId, range = 'A1:Z10000') {
  if (!ATIVO) return { simulado: true, linhas: [] };
  const tok = await accessToken();
  const url = `https://sheets.googleapis.com/v4/spreadsheets/${encodeURIComponent(spreadsheetId)}/values/${encodeURIComponent(range)}`;
  const res = await fetch(url, { headers: { Authorization: `Bearer ${tok}` } });
  if (!res.ok) throw new Error('Google Sheets: ' + await res.text());
  const j = await res.json();
  return { simulado: false, linhas: j.values || [] };
}

async function deletarEvento(calendarId, eventId) {
  if (!ATIVO) return { simulado: true };
  const tok = await accessToken();
  ...
  // 410/404 = já removido; tratamos como sucesso
  if (!res.ok && res.status !== 410 && res.status !== 404) throw new Error('Google delete: ' + await res.text());
  return { ok: true };
}

module.exports = { ATIVO, USA_SA, listarEventos, freeBusy, temConflito, criarEvento, deletarEvento, lerPlanilha };
