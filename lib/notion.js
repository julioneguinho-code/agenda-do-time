...
async function editarVenda(session, { id, nome, cpf, valor, grupo, cota, data, status, tipo }) {
  if (DEMO) {
...
async function muralDados(session, { mes } = {}) {
  ...
  const ranking = users.map(u => {
    ...
    return { email: u.email, nome: u.nome, primeiro: u.nome.split(' ')[0], time: u.time || '', foto: u.foto || '', cor: u.cor || '', vendido: Math.round(vendido * 100) / 100, meta, pct, qtd: fs.length, selo: seloMural(pct) };
