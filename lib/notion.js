...
async function clientesPorConsultor(session) {
  ...
  const base = await listarVendas(session, {});
  const vendas = (base.vendas || []).filter(v => v.status === 'fechada'); // só vendas fechadas
  const byCons = {};
  vendas.forEach(v => {
...
async function importarClientesXlsx(session, { arquivo, consultorEmail } = {}) {
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    const body = {
      nome, cpf: iCpf >= 0 ? normDocImport(row[iCpf]) : '',
      valor, grupo, cota, tamanhoGrupo: tamanho || undefined,
      tipo: iTipo >= 0 ? tipoImport(row[iTipo]) : '',
      data: (iData >= 0 ? parseDataImport(row[iData]) : '') || hoje(),
      status, _import: true, // importação é tolerante: aceita documento como veio na planilha
    };
    // dono da carteira: coluna Consultor na planilha tem prioridade; senão o consultor escolhido pelo gestor
    if (session.papel === 'gestor') { const em = (iCons >= 0 ? (row[iCons] || '').toString().trim() : '') || donoPadrao; if (em) body.consultorEmail = em; }
  ...
  return { ok: true, importados, falhas: erros.length, erros: erros.slice(0, 30) };
}
...
async function criarVenda(session, { nome, cpf, valor, grupo, cota, data, status, consultorEmail, tipo, tamanhoGrupo, _import }) {
