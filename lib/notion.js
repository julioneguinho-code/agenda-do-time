...
async function homeConsultor(session) {
  if (DEMO) {
    ...
    let cor = '', cargo = '', metaVenda = 0;
    try { const u = require('./auth').listarUsuarios().find(x => x.email === session.email); if (u) { cor = u.cor || ''; cargo = u.cargo || ''; metaVenda = +u.metaVenda || 0; } } catch (e) {}
...
async function dashboardVendas(session, { mes } = {}) {
  ...
  let users = [];
  try { users = require('./auth').listarUsuarios().filter(u => u.papel === 'consultor' && u.ativo !== false && (isMaster(session) || !session.time || u.time === session.time)); } catch (e) {}
  const consultores = users.map(u => {
    ...
    const metaV = +u.metaVenda || 0;
    return { nome, time: u.time || '', cor: u.cor || '', total: Math.round(total * 100) / 100, meta: metaV, pct: metaV ? Math.min(100, Math.round(100 * total / metaV)) : 0, qtd: fechadas.length, negociando: emNeg.reduce((s, v) => s + (+v.valor || 0), 0), bateu: metaV > 0 && total >= metaV };
