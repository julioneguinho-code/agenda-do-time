...
if (PROD) {
  ...
  demo.contratacao = { spreadsheetId: '', marcados: {} }; demo.vendas = []; demo.vendasLog = []; demo.cadastros = [];
}
...
async function solicitarReuniaoGestor(session, { paraEmail, assunto, inicio, duracao, local }) {
  ...
  if (DEMO) {
    const nova = {
      id: 's' + Date.now(), assunto: assunto || 'Reunião', consultor: session.nome.split(' ')[0],
...
async function painelGestor(session) {
  if (DEMO) {
    ...
    return {
      ...
      execucao: (() => { const c = coresConsultores(); return nomes.map(n => ({ nome: n, pct: execPct(n), cor: (c[n] || {}).cor || '', cargo: (c[n] || {}).cargo || '', foto: fotoConsultorTime(session, n) })).sort((a, b) => b.pct - a.pct); })(),
      pendentes: (() => { const c = coresConsultores(); return demo.solicitacoes.filter(s => s.status === 'Pendente' && visivelPara(session, s)).map(s => ({ ...s, cor: (c[s.consultor] || {}).cor || '', cargo: (c[s.consultor] || {}).cargo || '', foto: fotoConsultorTime(session, s.consultor) })); })(),
      ...
      atividadesAtrasadas: demo.atividades.filter(a => a.status !== 'Feito' && a.status !== 'Cancelada' && a.prazo < hoje()).map(a => ({ ...a, foto: fotoConsultorTime(session, a.consultor) })),
      consultores: nomes,
...
async function decidirSolicitacao(session, { id, decisao, resposta, paraTime, paraEmail, paraNome }) {
  if (DEMO) {
    ...
    if (s) {
      ...
      else novoAviso(s.consultor, `Reunião ${decisao.toLowerCase()}`, msg);
...
async function rankingGestor(session) {
  if (DEMO) {
    ...
    const rk = users.map(u => {
      ...
      return { nome, foto: u.foto || '', time: u.time || '', cor: u.cor || '', vendido: Math.round(vendido * 100) / 100, metaVenda: metaV, vendasPct, checkAtual: prog, checkAlvo: demo.metaSemanal, checkPct, pontos };
...
async function controleGestor(session) {
  if (DEMO) {
    ...
    const consultores = nomes.map(nome => {
      ...
      return {
        nome,
        foto: fotoConsultorTime(session, nome),
        cor: (cores[nome] || {}).cor || '',
...
async function perfilConsultor(session, nome) {
  if (DEMO) {
    ...
    return {
      nome, time, foto: fotoConsultorTime(session, nome), execucao: exec, cor: info.cor || '', cargo: info.cargo || '',
...
async function listarAvisos(session) {
  const chaves = session.papel === 'gestor' ? ['gestor:' + (session.time || ''), 'email:' + session.email, 'gestores'] : [session.nome.split(' ')[0]];
...
async function listarVendas(session, { de, ate, q, consultor } = {}) {
  if (DEMO) {
    ...
    if (session.papel === 'consultor') v = v.filter(x => x.consultorEmail === session.email);
      ...
      if (consultor) { const sel = String(consultor).split(',').map(s => s.trim()).filter(Boolean); if (sel.length) v = v.filter(x => sel.includes(x.consultor)); }
    }
...
async function criarVenda(session, { nome, cpf, valor, grupo, cota, data, status, consultorEmail }) {
  ...
  if (DEMO) {
    ...
    if (cpfDig.length !== 11) return { erro: 'Informe o CPF do cliente (11 dígitos)' };
    ...
    const venda = { id: 'v' + Date.now() + Math.random().toString(36).slice(2, 5), consultor: dono.nome.split(' ')[0], consultorEmail: dono.email, time: dono.time, cliente: { nome: String(nome).slice(0, 80), cpf: String(cpf || '').replace(/\D/g, '').slice(0, 11) }, valor: Math.round((+valor || 0) * 100) / 100, grupo: String(grupo || '').slice(0, 40), cota: String(cota || '').slice(0, 40), data: data || hoje(), status: status === 'fechada' ? 'fechada' : 'negociacao', criadoEm: new Date().toISOString(), criadoPor: session.nome ? session.nome.split(' ')[0] : session.email };
...
async function editarVenda(session, { id, nome, cpf, valor, grupo, cota, data, status }) {
  if (DEMO) {
    ...
    if (cpf != null && String(cpf).replace(/\D/g, '').length !== 11) return { erro: 'Informe o CPF do cliente (11 dígitos)' };
