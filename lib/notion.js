...
async function rankingGestor(session) {
  if (DEMO) {
    ...
    const rk = users.map(u => {
      ...
      return { nome, time: u.time || '', cor: u.cor || '', vendido: Math.round(vendido * 100) / 100, metaVenda: metaV, vendasPct, checkAtual: prog, checkAlvo: demo.metaSemanal, checkPct, pontos };
...
async function dashboardVendas(session, { mes } = {}) {
  ...
  const consultores = users.map(u => {
    ...
    const metaV = +u.metaVenda || 0;
    return { nome, email: u.email, papel: u.papel, time: u.time || '', cor: u.cor || '', total: Math.round(total * 100) / 100, meta: metaV, pct: metaV ? Math.min(100, Math.round(100 * total / metaV)) : 0, qtd: fechadas.length, negociando: emNeg.reduce((s, v) => s + (+v.valor || 0), 0), bateu: metaV > 0 && total >= metaV };
