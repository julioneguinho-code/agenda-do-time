...
async function negociacoesTodasEquipes(session) {
  ...
  if (DEMO) {
    ...
    const byEmail = {}; users.forEach(u => { byEmail[String(u.email).toLowerCase()] = { nome: u.nome.split(' ')[0], time: u.time || '', foto: u.foto || '', cor: u.cor || '' }; });
