...
async function painelGestor(session) {
  if (DEMO) {
    ...
    return {
      ...
      decididas: (() => { const c = coresConsultores(); return demo.solicitacoes.filter(s => ['Aprovada', 'Recusada', 'Reagendar'].includes(s.status)).slice(0, 10).map(s => ({ ...s, cor: (c[s.consultor] || {}).cor || '' })); })(),
