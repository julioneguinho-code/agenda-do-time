...
const COLS_MODELO = ['Cliente', 'CPF', 'Grupo', 'Cota', 'Tamanho do grupo', 'Tipo (veiculo/imovel/servicos)', 'Valor', 'Data (dd/mm/aaaa)', 'Status (fechada/negociacao)'];
function modeloClientesXlsx() {
  const xlsx = require('./xlsx');
  const ex1 = ['Ana Paula Souza', '12345678909', '860', '511', 2000, 'imovel', 346350.63, '20/08/2026', 'fechada'];
  const ex2 = ['Bruno Lima', '98765432100', '900', '12', 1000, 'veiculo', 50000, '15/07/2026', 'fechada'];
  const buf = xlsx.build(COLS_MODELO, [ex1, ex2], { moneyCols: [6], sheetName: 'Clientes', widths: [24, 15, 10, 10, 16, 24, 14, 16, 22] });
...
async function importarClientesXlsx(session, { arquivo, consultorEmail } = {}) {
  ...
  const iStatus = acha('status', 'situacao');
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    status = /fech|efetiv|ativ/.test(status) ? 'fechada' : (/negoc/.test(status) ? 'negociacao' : (grupo && cota && tamanho ? 'fechada' : 'negociacao'));
    ...
    const body = { nome, cpf: cpfDig, valor, grupo, cota, tamanhoGrupo: tamanho || undefined, tipo, data: data || hoje(), status, _import: true, carteira: true };
...
async function criarVenda(session, { nome, cpf, valor, grupo, cota, data, status, consultorEmail, tipo, tamanhoGrupo, _import, carteira }) {
...
async function comImportar(session, { arquivo, competencia } = {}) {
  ...
  const comp = /^\d{4}-\d{2}$/.test(String(competencia || '')) ? competencia : new Date().toISOString().slice(0, 7);
  const fech = {
