...
async function listarVendas(session, { de, ate, q, consultor } = {}) {
  if (DEMO) {
    ...
    if (session.papel === 'consultor') v = v.filter(x => x.consultorEmail === session.email);
      ...
      v = v.filter(x => isMaster(session) || nomes.includes(x.consultor) || x.consultorEmail === session.email);
...
async function dashboardVendas(session, { mes } = {}) {
  ...
  const consultores = users.map(u => {
    ...
    const metaV = +u.metaVenda || 0;
    return { nome, papel: u.papel, time: u.time || '', cor: u.cor || '', total: Math.round(total * 100) / 100, meta: metaV, pct: metaV ? Math.min(100, Math.round(100 * total / metaV)) : 0, qtd: fechadas.length, negociando: emNeg.reduce((s, v) => s + (+v.valor || 0), 0), bateu: metaV > 0 && total >= metaV };
