...
async function criarVenda(session, { nome, cpf, valor, grupo, cota, data }) {
  ...
  if (DEMO) {
    ...
    const venda = { id: 'v' + Date.now() + Math.random().toString(36).slice(2, 5), consultor: session.nome.split(' ')[0], consultorEmail: session.email, time: session.time || '', cliente: { nome: String(nome).slice(0, 80), cpf: String(cpf || '').replace(/\D/g, '').slice(0, 11) }, valor: Math.round((+valor || 0) * 100) / 100, grupo: String(grupo || '').slice(0, 40), cota: String(cota || '').slice(0, 40), data: data || hoje(), criadoEm: new Date().toISOString() };
    demo.vendas.unshift(venda); return { ok: true, venda };
  }
  return { ok: true };
}
async function editarVenda(session, { id, nome, cpf, valor, grupo, cota, data }) {
  if (DEMO) {
