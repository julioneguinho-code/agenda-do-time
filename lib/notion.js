...
async function importarClientesXlsx(session, { arquivo } = {}) {
  if (session.papel !== 'gestor' && session.papel !== 'consultor') return { erro: 'Sem permissão' };
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    const body = {
      nome, cpf: iCpf >= 0 ? (row[iCpf] || '').toString() : '',
    ...
    if (session.papel === 'gestor' && iCons >= 0) { const em = (row[iCons] || '').toString().trim(); if (em) body.consultorEmail = em; }
...
async function criarVenda(session, { nome, cpf, valor, grupo, cota, data, status, consultorEmail, tipo, tamanhoGrupo }) {
  ...
  if (DEMO) {
    ...
    if (vaiFechar && (!String(grupo || '').trim() || !String(cota || '').trim() || !tamanho))
