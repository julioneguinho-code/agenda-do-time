...
async function painelGestor(session) {
  if (DEMO) {
    ...
    return {
      ...
      execucao: cts.map(u => { const l = String(u.email).toLowerCase(); return { nome: u.nome.split(' ')[0], email: l, pct: execPct(l), cor: u.cor || '', cargo: u.cargo || '', foto: u.foto || '' }; }).sort((a, b) => b.pct - a.pct),
      pendentes: demo.solicitacoes.filter(s => s.status === 'Pendente' && visivelPara(session, s)).map(s => { const cc = corCargoDe(s.consultorEmail); return { ...s, cor: cc.cor, cargo: cc.cargo, foto: fotoDeLogin(s.consultorEmail) }; }),
      decididas: demo.solicitacoes.filter(s => ['Aprovada', 'Recusada', 'Reagendar'].includes(s.status) && visivelPara(session, s)).slice(0, 10).map(s => ({ ...s, cor: corCargoDe(s.consultorEmail).cor })),
      atividadesAtrasadas: demo.atividades.filter(a => a.status !== 'Feito' && a.status !== 'Cancelada' && a.prazo < hoje() && logins.includes(String(a.consultorEmail || '').toLowerCase())).map(a => ({ ...a, foto: fotoDeLogin(a.consultorEmail) })),
      consultores: cts.map(u => ({ nome: u.nome.split(' ')[0], email: String(u.email).toLowerCase() })),
...
async function perfilConsultor(session, ident) {
  if (DEMO) {
    ...
    const nome = u ? u.nome.split(' ')[0] : String(ident || '');
...
async function listarChat(session, consultorParam) {
  ...
  if (!uid) {
    ...
    return { lista: consultoresTime(session).map(u => { const l = String(u.email).toLowerCase(); return { nome: u.nome.split(' ')[0], email: l, foto: u.foto || '', naoLidas: demo.mensagens.filter(m => String(m.consultorEmail || '').toLowerCase() === l && m.de === 'consultor' && !m.lida).length }; }) };
  }
  ...
  return { consultor: nomeCurtoDe(uid), email: uid, mensagens: thread };
}
