...
async function sortearProximidade(session, { premios, grupo, tamanho } = {}) {
  ...
  const cotasGrupo = (base.vendas || []).filter(v => v.status === 'fechada' && String(v.grupo || '').trim() === grupoAlvo);
...
async function editarVenda(session, { id, nome, cpf, valor, grupo, cota, data, status, tipo, consultorEmail, tamanhoGrupo }) {
  if (DEMO) {
    ...
    if (cpf != null && cpfFinal && !docValido(cpfFinal)) return { erro: 'CPF/CNPJ inválido — confira os números' };
    // status final
    const statusFinal = status != null ? (status === 'fechada' ? 'fechada' : 'negociacao') : v.status;
    if (statusFinal === 'fechada' && !docValido(cpfFinal)) return { erro: 'Para efetivar a venda, informe um CPF ou CNPJ válido do cliente' };
    ...
    if (statusFinal === 'fechada' && (!grupoFinal || !cotaFinal || !tamanhoFinal))
      return { erro: 'Para efetivar a venda, informe o grupo, a cota e o tamanho do grupo' };
