...
async function listarArquivos(session) {
  ...
  const lista = (demo.arquivos || []).filter(a => podeVerArquivo(session, a))
    .map(a => ({ id: a.id, titulo: a.titulo, nome: a.nome, tipo: a.tipo, tamanho: a.tamanho, destino: a.destino, criadoPor: a.criadoPor, data: a.data, expiraEm: new Date(new Date(a.data).getTime() + ARQUIVO_TTL).toISOString() }));
