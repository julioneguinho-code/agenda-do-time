...
// Datas no fuso de Brasília (America/Sao_Paulo) — evita "virar o dia/mês" às 21h por causa do UTC.
// Declarado no topo porque o objeto `demo` (mais abaixo) já usa anoAtualBR() na sua inicialização.
const TZ_BR = 'America/Sao_Paulo';
function hoje() { return new Date().toLocaleDateString('en-CA', { timeZone: TZ_BR }); } // YYYY-MM-DD
function mesAtualBR() { return hoje().slice(0, 7); }  // YYYY-MM
function anoAtualBR() { return +hoje().slice(0, 4); }
...
const demo = {
  ...
  mural: { ano: anoAtualBR(), desafio: 0, expectativa: 0, piso: 0, recado: '', metasTime: {} }, // config do Mural (só gestor edita)
  comImpostos: {}, // por gestorEmail -> [{id,nome,tipo:'pct'|'fixo',valor}]
  comFechamentos: [], // histórico de importações de comissão (repasse)
};
...
function lerEstadoArquivo(f) {
  const s = JSON.parse(fsp.readFileSync(f, 'utf8'));
...
function carregarEstado() {
  try {
    if (fsp.existsSync(ESTADO_FILE)) {
      try { lerEstadoArquivo(ESTADO_FILE); }
      catch (e1) {
        // arquivo principal corrompido → tenta o backup rotativo
        console.error('estado.json corrompido, tentando backup:', e1.message);
        if (fsp.existsSync(ESTADO_FILE + '.bak')) { lerEstadoArquivo(ESTADO_FILE + '.bak'); console.warn('estado recuperado do backup .bak'); }
      }
    }
...
function salvarEstado() {
  try {
    fsp.mkdirSync(DATA_DIR, { recursive: true });
    const dados = JSON.stringify(serializarEstado());
    // Escrita ATÔMICA: grava em arquivo temporário e renomeia (rename é atômico no mesmo
    // volume). Assim, se o processo cair no meio da escrita, o estado.json antigo continua íntegro.
    const tmp = ESTADO_FILE + '.tmp';
    fsp.writeFileSync(tmp, dados);
    fsp.renameSync(tmp, ESTADO_FILE);
    // Backup rotativo: uma cópia de segurança a cada ~10 min (recuperação se o arquivo corromper).
    const agora = Date.now();
    if (agora - _ultimoBackup > 10 * 60000) {
      try { fsp.copyFileSync(ESTADO_FILE, ESTADO_FILE + '.bak'); _ultimoBackup = agora; } catch (e) {}
    }
...
function isMaster(session) {
  ...
  if (String(session.email || '').toLowerCase() === 'gestorchama') return true;
  // usa o cadastro ATUAL (evita flag "master" desatualizada no token da sessão)
  try { const u = require('./auth').listarUsuarios().find(x => String(x.email).toLowerCase() === String(session.email || '').toLowerCase()); if (u) return u.master === true; } catch (e) {}
  return session.master === true;
}
// time ATUAL do usuário (do cadastro), não confia no token que pode estar velho
function timeAtualDe(session) {
  try { const u = require('./auth').listarUsuarios().find(x => String(x.email).toLowerCase() === String(session && session.email || '').toLowerCase()); if (u) return u.time || ''; } catch (e) {}
  return (session && session.time) || '';
}
function nomesDoTime(session) {
  try {
    const auth = require('./auth');
    const t = timeAtualDe(session);
    const us = auth.listarUsuarios().filter(u => u.papel === 'consultor' && u.ativo !== false && (isMaster(session) || (t && u.time === t)));
...
function fotoConsultorTime(session, primeiroNome) {
  try {
    ...
    const t = timeAtualDe(session);
    const u = us.find(x => x.nome.split(' ')[0] === primeiroNome && (isMaster(session) || (t && x.time === t)));
    return u ? (u.foto || '') : '';
  } catch (e) { return ''; }
}
...
function consultoresTime(session) { // usuários consultores no escopo do gestor
  try { const t = timeAtualDe(session); return require('./auth').listarUsuarios().filter(u => u.papel === 'consultor' && u.ativo !== false && (isMaster(session) || (t && u.time === t))); } catch (e) { return []; }
}
...
function visivelPara(session, s) {
  ...
  const t = timeAtualDe(session); return !!t && t === s.time;
}
function meuTime(session, timeDaSolicitacao) { const t = timeAtualDe(session); return !!t && t === timeDaSolicitacao; }
...
async function homeConsultor(session) {
  if (DEMO) {
    ...
    const vendidoMes = (demo.vendas || []).filter(v => v.consultorEmail === session.email && !v.carteira && v.status === 'fechada' && String(v.data || '').slice(0, 7) === mesAtual).reduce((s, v) => s + (+v.valor || 0), 0);
    return {
      nome, cor, cargo, foto,
...
async function destinatarios(session) {
  try {
    ...
    const tD = timeAtualDe(session);
    const consultores = us.filter(u => u.papel === 'consultor' && (isMaster(session) || (tD && u.time === tD))).map(u => ({ nome: u.nome.split(' ')[0], nomeCompleto: u.nome, login: String(u.email).toLowerCase(), papel: 'consultor', time: u.time || '' }));
...
async function cancelarSolicitacao(session, { id }) {
  if (DEMO) {
    const s = demo.solicitacoes.find(x => x.id === id && x.consultorEmail === loginDe(session));
...
async function excluirSolicitacao(session, { id }) {
  if (DEMO) {
    const uid = loginDe(session);
    const i = demo.solicitacoes.findIndex(x => x.id === id && x.consultorEmail === uid);
...
async function rankingGestor(session) {
  if (DEMO) {
    ...
    const rk = users.map(u => {
      ...
      const vendido = (demo.vendas || []).filter(v => v.consultorEmail === u.email && !v.carteira && v.status === 'fechada' && String(v.data || '').slice(0, 7) === mesAtual).reduce((s, v) => s + (+v.valor || 0), 0);
...
function usuariosDoTime(session) { // usuários (consultores+gestores) visíveis pelo gestor
  // master vê tudo; senão SÓ o mesmo time (nunca "tudo" quando o time está vazio)
  try { if (isMaster(session)) return require('./auth').listarUsuarios(); const t = timeAtualDe(session); if (!t) return []; return require('./auth').listarUsuarios().filter(u => u.time === t); } catch (e) { return []; }
}
...
function emailsDoTime(session) { // emails do time + o próprio gestor (para checar dono da venda)
  const s = new Set(usuariosDoTime(session).map(u => String(u.email || '').toLowerCase()));
  if (session && session.email) s.add(String(session.email).toLowerCase());
  return s;
}
...
function cnpjValido(cnpj) {
  ...
  const calc = (base) => { let s = 0, p = base.length - 7; for (let i = 0; i < base.length; i++) { s += parseInt(base[i], 10) * p; p--; if (p < 2) p = 9; } const r = s % 11; return r < 2 ? 0 : 11 - r; };
  const d1 = calc(cnpj.slice(0, 12)); if (d1 !== parseInt(cnpj[12], 10)) return false;
  const d2c = calc(cnpj.slice(0, 13)); if (d2c !== parseInt(cnpj[13], 10)) return false;
  return true;
}
// aceita CPF (11) ou CNPJ (14)
function docValido(v) { const d = String(v || '').replace(/\D/g, ''); return d.length === 14 ? cnpjValido(d) : cpfValido(d); }
...
async function listarVendas(session, { de, ate, q, consultor } = {}) {
  if (DEMO) {
    ...
    else { // gestor: vê SÓ o próprio time, por identidade (e-mail) — nunca por nome (evita vazar entre equipes)
      const emails = emailsDoTime(session);
      v = v.filter(x => isMaster(session) || emails.has(String(x.consultorEmail || '').toLowerCase()));
...
async function exportarVendasXlsx(session, { de, ate, q, consultor } = {}) {
  ...
  const vendas = (base.vendas || []).filter(v => !v.carteira); // exporta só Vendas (carteira fica na Gestão de cliente)
...
// ===== SORTEIO POR PROXIMIDADE (Loteria Federal) =====
// número sorteado do grupo = últimos 4 dígitos do prêmio, "dando a volta" no tamanho N (0 => N)
function numeroSorteadoGrupo(premio, N) {
  const base = parseInt(String(premio).replace(/\D/g, '').slice(-4) || '0', 10) % 10000;
  const r = base % N;
  return r === 0 ? N : r;
}
// "casa" = distância na caminhada alternada (1 p/ frente, 1 p/ trás, 2 p/ frente...) num anel de N cotas
function casaDaCota(cota, sorteado, N) {
  const d = ((cota - sorteado) % N + N) % N; // passos p/ frente (0..N-1)
  if (d === 0) return 0;
  return Math.min(2 * d - 1, 2 * (N - d)); // frente = ímpar, trás = par; pega o menor caminho
}
// lista as nossas cotas de um grupo ordenadas por proximidade aos números sorteados (5 prêmios da Federal)
async function sortearProximidade(session, { premios, grupo, tamanho } = {}) {
  if (session.papel !== 'gestor' && session.papel !== 'consultor') return { erro: 'Sem permissão' };
  const grupoAlvo = String(grupo || '').trim();
  if (!grupoAlvo) return { erro: 'Escolha o grupo' };
  // prêmios válidos (guarda o número do prêmio: 1º..5º)
  const listaPremios = (Array.isArray(premios) ? premios : []).map((p, i) => ({ premio: i + 1, num: String(p || '').replace(/\D/g, '') })).filter(p => p.num.length >= 1);
  if (!listaPremios.length) return { erro: 'Digite ao menos um número da Loteria Federal' };
  // nossas cotas fechadas desse grupo (mesma visibilidade da listagem de vendas)
  const base = await listarVendas(session, {});
  const cotasGrupo = (base.vendas || []).filter(v => (v.carteira || v.status === 'fechada') && v.situacao !== 'cancelado' && v.situacao !== 'contemplado' && String(v.grupo || '').trim() === grupoAlvo);
  // tamanho N: o informado, senão o mais comum entre as cotas do grupo
  let N = tamanhoGrupoValido(tamanho);
  if (!N) { const cont = {}; cotasGrupo.forEach(v => { const t = tamanhoGrupoValido(v.tamanhoGrupo); if (t) cont[t] = (cont[t] || 0) + 1; }); N = +Object.keys(cont).sort((a, b) => cont[b] - cont[a])[0] || 0; }
  if (!N) return { erro: 'Tamanho do grupo não definido — cadastre o tamanho nas cotas' };
  // números sorteados de cada prêmio
  const sorteados = listaPremios.map(p => ({ premio: p.premio, entrada: p.num, numero: numeroSorteadoGrupo(p.num, N) }));
  // para cada cota, acha o prêmio mais próximo (menor casa)
  const itens = [];
  cotasGrupo.forEach(v => {
    const cotaNum = parseInt(String(v.cota || '').replace(/\D/g, ''), 10);
    if (!cotaNum || cotaNum > N) return; // cota inválida ou fora do grupo
    let melhor = null;
    sorteados.forEach(s => { const c = casaDaCota(cotaNum, s.numero, N); if (melhor === null || c < melhor.casa) melhor = { casa: c, premio: s.premio, sorteado: s.numero }; });
    itens.push({
      cliente: (v.cliente && v.cliente.nome) || '', cpf: (v.cliente && v.cliente.cpf) || '',
      consultor: v.consultor || '', grupo: v.grupo || '', cota: cotaNum, valor: +v.valor || 0,
      tipo: v.tipo || '', data: v.data || '', casa: melhor.casa, premio: melhor.premio, sorteado: melhor.sorteado,
    });
  });
  itens.sort((a, b) => a.casa - b.casa || a.cota - b.cota);
  return { ok: true, grupo: grupoAlvo, tamanho: N, sorteados, itens, qtd: itens.length };
}
// grupos disponíveis para sorteio (nossas cotas fechadas com tamanho definido)
async function gruposParaSorteio(session) {
  if (session.papel !== 'gestor' && session.papel !== 'consultor') return { erro: 'Sem permissão' };
  const base = await listarVendas(session, {});
  const map = {};
  (base.vendas || []).forEach(v => {
    if (!(v.carteira || v.status === 'fechada')) return;
    if (v.situacao === 'cancelado' || v.situacao === 'contemplado') return; // só ativas concorrem
    const g = String(v.grupo || '').trim(); if (!g) return;
    const t = tamanhoGrupoValido(v.tamanhoGrupo);
    if (!map[g]) map[g] = { grupo: g, tamanho: t || 0, qtd: 0 };
    map[g].qtd++; if (t && !map[g].tamanho) map[g].tamanho = t;
  });
  const grupos = Object.values(map).filter(g => g.tamanho).map(g => ({
    ...g,
    pct: g.tamanho ? Math.min(100, Math.round(100 * g.qtd / g.tamanho)) : 0,
    lotado: g.tamanho ? (g.qtd >= g.tamanho) : false,
  })).sort((a, b) => a.grupo.localeCompare(b.grupo, 'pt', { numeric: true }));
  return { grupos };
}
...
// clientes agrupados POR CONSULTOR (gestor): cada pessoa do time (inclui o gestor) com seus clientes/cotas
async function clientesPorConsultor(session) {
  if (session.papel !== 'gestor') return { erro: 'Somente gestores' };
  const r2 = x => Math.round((+x || 0) * 100) / 100;
  const base = await listarVendas(session, {});
  const vendas = (base.vendas || []).filter(v => v.carteira || v.status === 'fechada'); // carteira + vendas fechadas
  const byCons = {};
  vendas.forEach(v => {
    const email = String(v.consultorEmail || '').toLowerCase();
    if (!byCons[email]) byCons[email] = { email, nome: v.consultor || nomeCompletoDe(email), foto: fotoDeLogin(email), cor: corCargoDe(email).cor || '', time: v.time || '', vendas: [], cli: {} };
    const g = byCons[email];
    g.vendas.push(v);
    const cpf = ((v.cliente && v.cliente.cpf) || '').replace(/\D/g, '');
    const key = cpf ? 'cpf:' + cpf : 'nome:' + (((v.cliente && v.cliente.nome) || '').trim().toLowerCase());
    if (!g.cli[key]) g.cli[key] = { cpf, nome: (v.cliente && v.cliente.nome) || '—', vendas: [] };
    g.cli[key].vendas.push(v);
    if (v.cliente && v.cliente.nome) g.cli[key].nome = v.cliente.nome;
    if (cpf) g.cli[key].cpf = cpf;
  });
  const consultores = Object.values(byCons).map(g => {
    const clientes = Object.values(g.cli).map(c => {
      const fechado = c.vendas.filter(v => v.status === 'fechada').reduce((s, v) => s + (+v.valor || 0), 0);
      const negoc = c.vendas.filter(v => v.status !== 'fechada').reduce((s, v) => s + (+v.valor || 0), 0);
      return { cpf: c.cpf, nome: c.nome, qtd: c.vendas.length, fechado: r2(fechado), negociacao: r2(negoc), total: r2(fechado + negoc), vendas: c.vendas };
    }).sort((a, b) => b.total - a.total);
    const totalFechado = g.vendas.filter(v => v.status === 'fechada').reduce((s, v) => s + (+v.valor || 0), 0);
    const totalNeg = g.vendas.filter(v => v.status !== 'fechada').reduce((s, v) => s + (+v.valor || 0), 0);
    return { email: g.email, nome: g.nome, foto: g.foto, cor: g.cor, time: g.time, qtdCotas: g.vendas.length, qtdClientes: clientes.length, totalFechado: r2(totalFechado), totalNeg: r2(totalNeg), clientes };
  }).sort((a, b) => b.totalFechado - a.totalFechado || b.qtdCotas - a.qtdCotas);
  return { consultores, qtdConsultores: consultores.length, qtdCotas: vendas.length };
}
// ===== IMPORTAÇÃO DE CLIENTES POR PLANILHA (.xlsx) =====
const COLS_MODELO = ['Cliente', 'CPF', 'Grupo', 'Cota', 'Tamanho do grupo', 'Tipo (veiculo/imovel/servicos)', 'Valor', 'Data (dd/mm/aaaa)', 'Status (fechada/negociacao)', 'Situação (ativo/cancelado/contemplado)'];
function modeloClientesXlsx() {
  const xlsx = require('./xlsx');
  const ex1 = ['Ana Paula Souza', '12345678909', '860', '511', 2000, 'imovel', 346350.63, '20/08/2026', 'fechada', 'ativo'];
  const ex2 = ['Bruno Lima', '98765432100', '900', '12', 1000, 'veiculo', 50000, '15/07/2026', 'fechada', 'contemplado'];
  const buf = xlsx.build(COLS_MODELO, [ex1, ex2], { moneyCols: [6], sheetName: 'Clientes', widths: [24, 15, 10, 10, 16, 24, 14, 16, 22, 26] });
  return { ok: true, buffer: buf, filename: 'modelo-clientes.xlsx' };
}
function normCab(s) { return String(s || '').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').trim(); }
function parseValorImport(v) { if (v == null || v === '') return 0; const s = String(v).trim(); if (s.includes(',')) return parseFloat(s.replace(/\./g, '').replace(',', '.')) || 0; return parseFloat(s.replace(/[^\d.]/g, '')) || 0; }
function parseDataImport(v) {
  if (v == null || v === '') return '';
  const s = String(v).trim();
  if (/^\d+(\.\d+)?$/.test(s) && +s > 1000) { const d = new Date(Date.UTC(1899, 11, 30)); d.setUTCDate(d.getUTCDate() + Math.floor(+s)); return d.toISOString().slice(0, 10); }
...
function tipoImport(v) { const s = normCab(v); if (/veic|carro|auto|moto/.test(s)) return 'veiculo'; if (/imov|casa|apart|terreno/.test(s)) return 'imovel'; if (/serv/.test(s)) return 'servicos'; return ''; }
// normaliza documento vindo da planilha: tira não-dígitos e recupera zero à esquerda que o Excel corta
function normDocImport(v) { let d = String(v == null ? '' : v).replace(/\D/g, ''); if (d.length === 10) d = '0' + d; if (d.length === 13) d = '0' + d; return d; }
async function importarClientesXlsx(session, { arquivo, consultorEmail } = {}) {
  if (session.papel !== 'gestor' && session.papel !== 'consultor') return { erro: 'Sem permissão' };
  // gestor pode escolher para QUAL consultor a carteira será atribuída (padrão da importação)
  let donoPadrao = '';
  if (session.papel === 'gestor' && consultorEmail) {
  ...
  if (!arquivo) return { erro: 'Nenhum arquivo enviado' };
  let rows;
  try { rows = require('./xlsx').read(Buffer.from(String(arquivo), 'base64')); } catch (e) { return { erro: 'Não foi possível ler a planilha (.xlsx): ' + (e.message || e) }; }
  if (!rows || rows.length < 2) return { erro: 'Planilha vazia — preencha ao menos uma linha' };
  const cab = (rows[0] || []).map(normCab);
  const acha = (...alts) => { for (let i = 0; i < cab.length; i++) { if (alts.some(a => cab[i].includes(a))) return i; } return -1; };
  const iNome = acha('cliente', 'nome');
  const iCpf = acha('cpf');
  const iGrupo = cab.findIndex(c => c === 'grupo' || (c.includes('grupo') && !c.includes('tamanho')));
  const iCota = acha('cota');
  const iTam = acha('tamanho', 'participante');
  const iTipo = acha('tipo', 'segmento');
  const iValor = acha('valor');
  const iData = acha('data');
  const iStatus = acha('status', 'estagio', 'etapa', 'fase'); // estágio da venda (fechada/negociação)
  const iSituacao = acha('situacao'); // situação da cota (ativo/cancelado/contemplado)
  const iCons = acha('consultor', 'vendedor', 'email');
  if (iNome < 0 || iValor < 0) return { erro: 'A planilha precisa ter ao menos as colunas Cliente e Valor. Use o modelo.' };
  let importados = 0, atualizados = 0; const erros = [];
  for (let r = 1; r < rows.length; r++) {
    const row = rows[r] || [];
    const nome = (row[iNome] || '').toString().trim();
    const valor = iValor >= 0 ? parseValorImport(row[iValor]) : 0;
    if (!nome && !valor) continue; // linha vazia
    const grupo = iGrupo >= 0 ? (row[iGrupo] || '').toString().trim() : '';
    const cota = iCota >= 0 ? (row[iCota] || '').toString().trim() : '';
    const tamanho = iTam >= 0 ? tamanhoGrupoValido((row[iTam] || '').toString().replace(/\D/g, '')) : 0;
    let status = iStatus >= 0 ? normCab(row[iStatus]) : '';
    status = /fech|efetiv/.test(status) ? 'fechada' : (/negoc/.test(status) ? 'negociacao' : (grupo && cota && tamanho ? 'fechada' : 'negociacao'));
    // Situação da cota (coluna "Situação"): Ativo / Cancelado / Contemplado (afeta o sorteio)
    const sitTxt = iSituacao >= 0 ? normCab(row[iSituacao]) : '';
    const situacao = /cancel/.test(sitTxt) ? 'cancelado' : (/contempl/.test(sitTxt) ? 'contemplado' : 'ativo');
    const cpfDig = iCpf >= 0 ? normDocImport(row[iCpf]) : '';
    const tipo = iTipo >= 0 ? tipoImport(row[iTipo]) : '';
    const data = (iData >= 0 ? parseDataImport(row[iData]) : '') || '';
    let consEmail = '';
    if (session.papel === 'gestor') consEmail = (iCons >= 0 ? (row[iCons] || '').toString().trim() : '') || donoPadrao;
    // JÁ EXISTE (mesmo grupo+cota)? Então SOBREPÕE/COMPLEMENTA em vez de duplicar
    const existente = (grupo && cota) ? (demo.vendas || []).find(v => chaveGC(v.grupo, v.cota) === chaveGC(grupo, cota)) : null;
    if (existente) {
      if (nome) existente.cliente.nome = nome.slice(0, 80);
      if (cpfDig) existente.cliente.cpf = cpfDig.slice(0, 14);
      if (valor) existente.valor = Math.round(valor * 100) / 100;
      if (tamanho) existente.tamanhoGrupo = tamanho;
      if (tipo) existente.tipo = tipo;
      if (data) existente.data = data;
      if (iSituacao >= 0) existente.situacao = situacao; // só sobrescreve se a planilha trouxe a coluna Situação
      if (consEmail && session.papel === 'gestor') { const u = usuariosDoTime(session).find(x => String(x.email).toLowerCase() === consEmail.toLowerCase()); if (u) { existente.consultorEmail = u.email; existente.consultor = u.nome.split(' ')[0]; existente.time = u.time || ''; } }
      existente.carteira = true; // continua sendo carteira
      atualizados++;
      continue;
    }
    const body = { nome, cpf: cpfDig, valor, grupo, cota, tamanhoGrupo: tamanho || undefined, tipo, data: data || hoje(), status, situacao, _import: true, carteira: true };
    if (consEmail) body.consultorEmail = consEmail;
    const res = await criarVenda(session, body);
    if (res && (res.ok || res.venda)) importados++;
    else erros.push({ linha: r + 1, erro: (res && res.erro) || 'Erro ao importar' });
  }
  if (atualizados) salvarEstado();
  return { ok: true, importados, atualizados, falhas: erros.length, erros: erros.slice(0, 30) };
}
...
async function dashboardVendas(session, { mes } = {}) {
  ...
  mes = mes || mesAtualBR();
  let users = [];
  try { const tD = timeAtualDe(session); users = require('./auth').listarUsuarios().filter(u => u.ativo !== false && ((u.papel === 'consultor' && (isMaster(session) || (tD && u.time === tD))) || (u.papel === 'gestor' && (isMaster(session) || u.email === session.email)))); } catch (e) {}
  const consultores = users.map(u => {
    const nome = u.nome;
    const fechadas = (demo.vendas || []).filter(v => v.consultorEmail === u.email && !v.carteira && v.status === 'fechada' && String(v.data || '').slice(0, 7) === mes);
    const emNeg = (demo.vendas || []).filter(v => v.consultorEmail === u.email && !v.carteira && v.status !== 'fechada' && String(v.data || '').slice(0, 7) === mes);
...
const TAMANHOS_GRUPO = [500, 1000, 2000, 3333, 5000, 9999];
function tamanhoGrupoValido(n) { const v = parseInt(n, 10); return TAMANHOS_GRUPO.includes(v) ? v : 0; }
async function criarVenda(session, { nome, cpf, valor, grupo, cota, data, status, consultorEmail, tipo, tamanhoGrupo, _import, carteira, situacao }) {
  ...
  if (DEMO) {
    ...
    const tamanho = tamanhoGrupoValido(tamanhoGrupo);
    // CPF é opcional na negociação, mas obrigatório e VÁLIDO para efetivar (fechar) a venda
    if (!_import && cpfDig && !docValido(cpfDig)) return { erro: 'CPF/CNPJ inválido — confira os números' };
    if (!_import && vaiFechar && !docValido(cpfDig)) return { erro: 'Para efetivar a venda, informe um CPF ou CNPJ válido do cliente' };
    // Para EFETIVAR (fechar): grupo, cota e tamanho do grupo são obrigatórios (usados no sorteio por proximidade)
    // Na importação (_import) NÃO trava: entra assim mesmo, o gestor completa depois.
    if (!_import && vaiFechar && (!String(grupo || '').trim() || !String(cota || '').trim() || !tamanho))
      return { erro: 'Para efetivar a venda, informe o grupo, a cota e o tamanho do grupo' };
    // NÃO DUPLICAR: se já existe uma cota com o mesmo grupo+cota, bloqueia (cota é única no consórcio)
    const _g = String(grupo || '').trim(), _c = String(cota || '').trim();
    if (_g && _c) {
      const k = chaveGC(_g, _c);
      if ((demo.vendas || []).some(v => chaveGC(v.grupo, v.cota) === k))
        return { erro: 'Cota já cadastrada (grupo ' + _g + ' · cota ' + _c + ')', duplicada: true };
    }
    ...
    const venda = { id: 'v' + Date.now() + Math.random().toString(36).slice(2, 5), consultor: dono.nome.split(' ')[0], consultorEmail: dono.email, time: dono.time, cliente: { nome: String(nome).slice(0, 80), cpf: String(cpf || '').replace(/\D/g, '').slice(0, 14) }, valor: Math.round((+valor || 0) * 100) / 100, grupo: String(grupo || '').slice(0, 40), cota: String(cota || '').slice(0, 40), tamanhoGrupo: tamanho || 0, tipo: tipoValido(tipo), data: data || hoje(), status: status === 'fechada' ? 'fechada' : 'negociacao', situacao: ['ativo', 'cancelado', 'contemplado'].includes(String(situacao)) ? String(situacao) : 'ativo', carteira: !!carteira, criadoEm: new Date().toISOString(), criadoPor: session.nome ? session.nome.split(' ')[0] : session.email };
    demo.vendas.unshift(venda);
    registrarCliente(venda);
    registrarLogVenda(session, 'incluiu', venda);
    // 🎉 venda já criada fechada por um consultor → avisa o gestor (carteira/cadastro de cliente NÃO comemora)
    const festa = venda.status === 'fechada' && !venda.carteira;
...
async function editarVenda(session, { id, nome, cpf, valor, grupo, cota, data, status, tipo, consultorEmail, tamanhoGrupo, situacao }) {
  if (DEMO) {
    ...
    if (!v.carteira && cpf != null && cpfFinal && !docValido(cpfFinal)) return { erro: 'CPF/CNPJ inválido — confira os números' };
    // status final
    const statusFinal = status != null ? (status === 'fechada' ? 'fechada' : 'negociacao') : v.status;
    if (!v.carteira && statusFinal === 'fechada' && !docValido(cpfFinal)) return { erro: 'Para efetivar a venda, informe um CPF ou CNPJ válido do cliente' };
    // valores finais de grupo/cota/tamanho (novo se enviado; senão o que já existe)
    const grupoFinal = grupo != null ? String(grupo).trim() : String(v.grupo || '').trim();
    const cotaFinal = cota != null ? String(cota).trim() : String(v.cota || '').trim();
    const tamanhoFinal = tamanhoGrupo != null ? tamanhoGrupoValido(tamanhoGrupo) : (v.tamanhoGrupo || 0);
    if (!v.carteira && statusFinal === 'fechada' && (!grupoFinal || !cotaFinal || !tamanhoFinal))
      return { erro: 'Para efetivar a venda, informe o grupo, a cota e o tamanho do grupo' };
    if (situacao != null) v.situacao = ['ativo', 'cancelado', 'contemplado'].includes(String(situacao)) ? String(situacao) : (v.situacao || 'ativo');
    if (nome != null) v.cliente.nome = String(nome).slice(0, 80);
    if (cpf != null) v.cliente.cpf = cpfFinal.slice(0, 14);
    ...
    if (tamanhoGrupo != null) v.tamanhoGrupo = tamanhoGrupoValido(tamanhoGrupo);
...
async function decidirCadastro(session, { id, aprovar, time }) {
  ...
  if (aprovar) {
    let rc;
    try {
      rc = require('./auth').criarUsuario(c.email, c.senha, c.nome, 'consultor', { time: String(time || c.time || session.time || '').slice(0, 40), ativo: true });
    } catch (e) { return { erro: 'Falha ao criar acesso: ' + e.message }; }
    if (rc && rc.erro) return { erro: rc.erro }; // login já existe — não aprova nem remove a solicitação
  }
...
async function muralDados(session, { mes } = {}) {
  const cfg = demo.mural || (demo.mural = { ano: anoAtualBR(), desafio: 0, expectativa: 0, piso: 0, recado: '', metasTime: {} });
  const ano = +cfg.ano || anoAtualBR();
  const mesAtual = mesAtualBR();
  ...
  try { users = require('./auth').listarUsuarios().filter(u => u.ativo !== false && (u.papel === 'consultor' || u.papel === 'gestor')); } catch (e) {}
  const fechadasDe = (email, ym) => (demo.vendas || []).filter(v => v.consultorEmail === email && !v.carteira && v.status === 'fechada' && String(v.data || '').slice(0, 7) === ym);
  ...
  const totalVendidoAno = (demo.vendas || []).filter(v => !v.carteira && v.status === 'fechada' && String(v.data || '').slice(0, 4) === String(ano)).reduce((s, v) => s + (+v.valor || 0), 0);
  const meses = [];
  for (let m = 1; m <= 12; m++) { const ym = ano + '-' + String(m).padStart(2, '0'); const val = (demo.vendas || []).filter(v => !v.carteira && v.status === 'fechada' && String(v.data || '').slice(0, 7) === ym).reduce((s, v) => s + (+v.valor || 0), 0); meses.push({ mes: ym, label: String(m).padStart(2, '0') + '/' + String(ano).slice(2), valor: Math.round(val * 100) / 100 }); }
  const timesMap = {};
...
async function muralSalvar(session, { desafio, expectativa, piso, recado, metasTime, ano }) {
  if (session.papel !== 'gestor') return { erro: 'Somente gestores' };
  demo.mural = demo.mural || { ano: anoAtualBR(), desafio: 0, expectativa: 0, piso: 0, recado: '', metasTime: {} };
  ...
  if (ano != null) demo.mural.ano = +ano || anoAtualBR();
...
// ===================== COMISSÕES / REPASSE (Ademicon) =====================
// chave de cruzamento: grupo+cota normalizados (tira não-dígitos e zeros à esquerda)
function chaveGC(grupo, cota) { const g = String(grupo || '').replace(/\D/g, '').replace(/^0+/, '') || '0'; const c = String(cota || '').replace(/\D/g, '').replace(/^0+/, '') || '0'; return g + '|' + c; }
function comParseValor(v) { if (v == null || v === '') return 0; const s = String(v).trim(); if (s.includes(',') && s.includes('.')) return parseFloat(s.replace(/\./g, '').replace(',', '.')) || 0; if (s.includes(',')) return parseFloat(s.replace(',', '.')) || 0; return parseFloat(s.replace(/[^\d.\-]/g, '')) || 0; }
function comImpostosDe(session) { demo.comImpostos = demo.comImpostos || {}; const k = String(session.email || '').toLowerCase(); return (demo.comImpostos[k] || []).map(x => ({ id: x.id, nome: x.nome, tipo: x.tipo === 'fixo' ? 'fixo' : 'pct', valor: +x.valor || 0 })); }
function comImpostosGet(session) { if (session.papel !== 'gestor') return { erro: 'Somente gestores' }; return { impostos: comImpostosDe(session) }; }
function comImpostosSalvar(session, { impostos } = {}) {
  if (session.papel !== 'gestor') return { erro: 'Somente gestores' };
  demo.comImpostos = demo.comImpostos || {};
  const k = String(session.email || '').toLowerCase();
  demo.comImpostos[k] = (Array.isArray(impostos) ? impostos : []).filter(x => x && String(x.nome || '').trim()).map(x => ({ id: x.id || ('imp' + Date.now() + Math.random().toString(36).slice(2, 5)), nome: String(x.nome).slice(0, 40), tipo: x.tipo === 'fixo' ? 'fixo' : 'pct', valor: Math.max(0, +x.valor || 0) }));
  salvarEstado();
  return { ok: true, impostos: comImpostosDe(session) };
}
// aplica impostos (pcts sobre a base + fixos rateados) e devolve o imposto de uma parcela "cheio" dado o total
function comCalcImposto(impostos, cheio, totalCheio) {
  let pct = 0, fixo = 0;
  (impostos || []).forEach(im => { if (im.tipo === 'fixo') fixo += +im.valor || 0; else pct += +im.valor || 0; });
  const porPct = cheio * pct / 100;
  const porFixo = totalCheio > 0 ? fixo * (cheio / totalCheio) : 0;
  return Math.round((porPct + porFixo) * 100) / 100;
}
// nível de comissão do gestor pela COR do cadastro: laranja (#FB923C) = supervisor 0,2%; qualquer
// outra cor = roxo 0,5% (só o roxo recebe o override integral). Case-insensitive.
const COR_LARANJA = '#FB923C';
function nivelPorCor(cor) { return String(cor || '').trim().toUpperCase() === COR_LARANJA ? 'laranja' : 'roxo'; }
// gestor (dono do nível) de um time — 1 gestor por time
function gestorDoTime(time) {
  const t = String(time || '').trim().toLowerCase();
  if (!t) return null;
  try { return require('./auth').listarUsuarios().find(u => u.papel === 'gestor' && String(u.time || '').trim().toLowerCase() === t) || null; }
  catch (e) { return null; }
}
// índice da carteira de TODOS os times (unificado) — usado só na atribuição de comissão
function indiceCarteiraUnificado() {
  const idx = {};
  (demo.vendas || []).forEach(v => { const k = chaveGC(v.grupo, v.cota); if (!idx[k]) idx[k] = v; });
  return idx;
}
async function comImportar(session, { arquivo, competencia } = {}) {
  if (session.papel !== 'gestor') return { erro: 'Somente gestores' };
  if (!arquivo) return { erro: 'Nenhum arquivo enviado' };
  let rows;
  try { rows = require('./xlsx').read(Buffer.from(String(arquivo), 'base64')); } catch (e) { return { erro: 'Não consegui ler o .xlsx: ' + (e.message || e) }; }
  if (!rows || rows.length < 2) return { erro: 'Planilha vazia' };
  const cab = (rows[0] || []).map(h => String(h || '').toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '').trim());
  const acha = (...alts) => { for (let i = 0; i < cab.length; i++) if (alts.some(a => cab[i].includes(a))) return i; return -1; };
  const iG = acha('grupo'), iC = acha('cota'), iV = acha('valor bruto', 'valor'), iCli = acha('nome do cliente', 'cliente'), iCt = acha('contrato'), iCat = acha('categoria'), iSeg = acha('segmento');
  if (iG < 0 || iC < 0 || iV < 0) return { erro: 'A planilha precisa ter Grupo, Cota e Valor bruto (use o relatório da Ademicon).' };
  if (iCat < 0) return { erro: 'A planilha precisa ter a coluna Categoria (AUTORIZADO / LICENCIADO).' };
  // agrupa por grupo+cota, separando AUTORIZADO (consultor) e LICENCIADO (gestor)
  const porCota = {};
  for (let r = 1; r < rows.length; r++) {
    const row = rows[r] || []; if (row[iG] == null && row[iC] == null) continue;
    const g = String(row[iG] || '').trim(), c = String(row[iC] || '').trim(); if (!g && !c) continue;
    const val = comParseValor(row[iV]);
    const cat = String(row[iCat] || '').toLowerCase();
    const ehLic = /licenc/.test(cat); // LICENCIADO -> gestor; senão AUTORIZADO -> consultor
    const key = chaveGC(g, c);
    if (!porCota[key]) porCota[key] = { grupo: g, cota: c, aut: 0, lic: 0, cliente: iCli >= 0 ? String(row[iCli] || '').trim() : '', contrato: iCt >= 0 ? String(row[iCt] || '').trim() : '', segmento: iSeg >= 0 ? String(row[iSeg] || '').trim() : '' };
    if (ehLic) porCota[key].lic += val; else porCota[key].aut += val;
  }
  // Índice UNIFICADO da carteira (todos os times) — para achar o dono real da cota e atribuir o repasse.
  const idx = indiceCarteiraUnificado();
  const r2 = n => Math.round(n * 100) / 100;
  // nível de quem está importando (pela cor do cadastro): roxo (0,5%) ou laranja (0,2%)
  let importador = null;
  try { importador = require('./auth').listarUsuarios().find(u => String(u.email).toLowerCase() === String(session.email || '').toLowerCase()); } catch (e) {}
  const nivelImportador = nivelPorCor(importador ? importador.cor : '');
  const meuEmail = String(session.email || '').toLowerCase();

  const porConsMap = {};   // AUTORIZADO por consultor (parte do consultor, inalterado)
  const repSupMap = {};    // repasses a supervisores laranja (0,2% = bruto/5*2)
  const repRoxoMap = {};   // repasses integrais a OUTROS gestores roxos (0,5%)
  const naoBateram = [];
  let gestorCheio = 0; const gestorCotas = []; // retenção de QUEM importou

  Object.values(porCota).forEach(rc => {
    const aut = r2(rc.aut), lic = r2(rc.lic);
    const nomeCli = k => { const v = idx[k]; return (v && v.cliente && v.cliente.nome) || rc.cliente || ''; };
    const k = chaveGC(rc.grupo, rc.cota);

    // ---- AUTORIZADO → consultor dono (busca unificada) ----
    if (aut > 0) {
      const v = idx[k];
      if (!v) { naoBateram.push({ grupo: rc.grupo, cota: rc.cota, cliente: rc.cliente, valor: aut, tipo: 'autorizado' }); }
      else {
        const em = String(v.consultorEmail || '').toLowerCase();
        if (!porConsMap[em]) porConsMap[em] = { email: em, nome: nomeCompletoDe(em) || v.consultor || em, cheio: 0, cotas: [] };
        porConsMap[em].cheio += aut;
        porConsMap[em].cotas.push({ grupo: rc.grupo, cota: rc.cota, cliente: (v.cliente && v.cliente.nome) || rc.cliente || '', valor: aut });
      }
    }

    // ---- LICENCIADO / override → depende do nível de quem importa ----
    if (lic > 0) {
      if (nivelImportador === 'laranja') {
        // supervisor: o valor da planilha JÁ é a parte dele (0,2%) — não divide, 100% dele
        gestorCheio += lic;
        gestorCotas.push({ grupo: rc.grupo, cota: rc.cota, cliente: nomeCli(k), valor: lic, origem: 'proprio' });
      } else {
        // ROXO importando: acha o dono e decide o split
        const v = idx[k];
        if (!v) { naoBateram.push({ grupo: rc.grupo, cota: rc.cota, cliente: rc.cliente, valor: lic, tipo: 'override' }); return; }
        const timeDono = v.time || '';
        const gd = gestorDoTime(timeDono);
        const nivelDono = nivelPorCor(gd ? gd.cor : '');
        const donoEmail = gd ? String(gd.email).toLowerCase() : '';
        if (nivelDono === 'laranja' && gd) {
          // cota de um time LARANJA → supervisor leva 2/5 (0,2%); roxo importador retém 3/5
          const cut = r2(lic / 5 * 2);
          const keep = r2(lic - cut);
          if (!repSupMap[donoEmail]) repSupMap[donoEmail] = { email: donoEmail, nome: gd.nome, time: timeDono, cheio: 0, cotas: [] };
          repSupMap[donoEmail].cheio += cut;
          repSupMap[donoEmail].cotas.push({ grupo: rc.grupo, cota: rc.cota, cliente: nomeCli(k), valor: cut, bruto: lic });
          gestorCheio += keep;
          gestorCotas.push({ grupo: rc.grupo, cota: rc.cota, cliente: nomeCli(k), valor: keep, origem: 'retido', bruto: lic });
        } else if (donoEmail && donoEmail !== meuEmail && gd) {
          // cota de OUTRO roxo → repasse integral (0,5%) para aquele gestor
          if (!repRoxoMap[donoEmail]) repRoxoMap[donoEmail] = { email: donoEmail, nome: gd.nome, time: timeDono, cheio: 0, cotas: [] };
          repRoxoMap[donoEmail].cheio += lic;
          repRoxoMap[donoEmail].cotas.push({ grupo: rc.grupo, cota: rc.cota, cliente: nomeCli(k), valor: lic });
        } else {
          // cota do PRÓPRIO time (ou sem gestor identificável) → roxo importador fica com 0,5% integral
          gestorCheio += lic;
          gestorCotas.push({ grupo: rc.grupo, cota: rc.cota, cliente: nomeCli(k), valor: lic, origem: 'proprio' });
        }
      }
  ...
  gestorCheio = r2(gestorCheio);
  const impostos = comImpostosDe(session);
  // base para ratear imposto fixo: soma de TODOS os cheios distribuídos
  const totalCheio = r2(gestorCheio
    + Object.values(porConsMap).reduce((s, x) => s + x.cheio, 0)
    + Object.values(repSupMap).reduce((s, x) => s + x.cheio, 0)
    + Object.values(repRoxoMap).reduce((s, x) => s + x.cheio, 0));
  const comLiq = (cheio) => { const imposto = comCalcImposto(impostos, r2(cheio), totalCheio); return { imposto, liquido: r2(cheio - imposto) }; };
  const mapParte = m => Object.values(m).map(p => { const cheio = r2(p.cheio); const { imposto, liquido } = comLiq(cheio); return { email: p.email, nome: p.nome, time: p.time || '', cheio, imposto, liquido, qtdCotas: p.cotas.length, cotas: p.cotas.sort((a, b) => b.valor - a.valor) }; }).sort((a, b) => b.liquido - a.liquido);

  const porConsultor = mapParte(porConsMap);
  const repassesSupervisor = mapParte(repSupMap);
  const repassesRoxo = mapParte(repRoxoMap);
  const gLiq = comLiq(gestorCheio);
  const gestor = { nome: session.nome || session.email, cheio: gestorCheio, imposto: gLiq.imposto, liquido: gLiq.liquido, nivel: nivelImportador, qtdCotas: gestorCotas.length, cotas: gestorCotas.sort((a, b) => b.valor - a.valor) };
  const totalImposto = r2(gestor.imposto + porConsultor.reduce((s, x) => s + x.imposto, 0) + repassesSupervisor.reduce((s, x) => s + x.imposto, 0) + repassesRoxo.reduce((s, x) => s + x.imposto, 0));
  // "a repassar" = tudo que SAI de quem importou (consultores + supervisores + outros roxos)
  const totalRepasse = r2(porConsultor.reduce((s, x) => s + x.liquido, 0) + repassesSupervisor.reduce((s, x) => s + x.liquido, 0) + repassesRoxo.reduce((s, x) => s + x.liquido, 0));
  const naoBateuTotal = r2(naoBateram.reduce((s, x) => s + x.valor, 0));
  const comp = /^\d{4}-\d{2}$/.test(String(competencia || '')) ? competencia : mesAtualBR();
  const fech = {
    id: 'f' + Date.now() + Math.random().toString(36).slice(2, 5),
    gestorEmail: meuEmail, gestorNome: session.nome || session.email, time: session.time || '', nivelImportador,
    competencia: comp, criadoEm: new Date().toISOString(),
    impostos, totalCheio, totalImposto, totalLiquido: totalRepasse, totalRepasse, qtdCotas: Object.keys(porCota).length,
    gestor, porConsultor, repassesSupervisor, repassesRoxo, naoBateram: naoBateram.sort((a, b) => b.valor - a.valor), naoBateuTotal,
  };
  demo.comFechamentos = demo.comFechamentos || [];
  demo.comFechamentos.unshift(fech);
  salvarEstado();
  return { ok: true, fechamento: fech };
}
function comPodeVer(session, f) { if (isMaster(session)) return true; return String(f.gestorEmail || '').toLowerCase() === String(session.email || '').toLowerCase(); }
function comListarFechamentos(session) {
  if (session.papel !== 'gestor') return { erro: 'Somente gestores' };
  const lista = (demo.comFechamentos || []).filter(f => comPodeVer(session, f)).map(f => ({ id: f.id, competencia: f.competencia, criadoEm: f.criadoEm, gestorNome: f.gestorNome, time: f.time, totalCheio: f.totalCheio, totalImposto: f.totalImposto, totalLiquido: f.totalLiquido, totalRepasse: f.totalRepasse != null ? f.totalRepasse : f.totalLiquido, seuLiquido: (f.gestor && f.gestor.liquido) || 0, qtdCotas: f.qtdCotas, naoBateu: (f.naoBateram || []).length }));
  return { fechamentos: lista, master: isMaster(session) };
}
function comFechamento(session, { id, consultor } = {}) {
  if (session.papel !== 'gestor') return { erro: 'Somente gestores' };
  const f = (demo.comFechamentos || []).find(x => x.id === id);
  if (!f || !comPodeVer(session, f)) return { erro: 'Fechamento não encontrado' };
  if (consultor) { const alvo = String(consultor).toLowerCase(); return { ...f, porConsultor: (f.porConsultor || []).filter(pc => pc.email === alvo) }; }
  return f;
}
function comExcluirFechamento(session, { id } = {}) {
  if (session.papel !== 'gestor') return { erro: 'Somente gestores' };
  const i = (demo.comFechamentos || []).findIndex(x => x.id === id);
  if (i < 0) return { erro: 'Não encontrado' };
  if (!comPodeVer(session, demo.comFechamentos[i])) return { erro: 'Sem permissão' };
  demo.comFechamentos.splice(i, 1); salvarEstado();
  return { ok: true };
}
// exporta um fechamento de comissão para .xlsx (resumo por consultor + detalhe das cotas)
function comExportarXlsx(session, { id } = {}) {
  if (session.papel !== 'gestor') return { erro: 'Somente gestores' };
  const f = (demo.comFechamentos || []).find(x => x.id === id);
  if (!f || !comPodeVer(session, f)) return { erro: 'Fechamento não encontrado' };
  const xlsx = require('./xlsx');
  const headers = ['Tipo', 'Nome', 'Grupo', 'Cota', 'Cliente', 'Valor cheio', 'Imposto', 'Líquido'];
  const rows = [];
  const g = f.gestor || { nome: f.gestorNome, cheio: 0, imposto: 0, liquido: 0, cotas: [] };
  const nivelLabel = (g.nivel || f.nivelImportador) === 'laranja' ? 'supervisor 0,2%' : 'gestor 0,5%';
  // QUEM IMPORTOU (retenção)
  rows.push(['Minha retenção (' + nivelLabel + ')', g.nome || f.gestorNome, '', '', '', g.cheio || 0, g.imposto || 0, g.liquido || 0]);
  (g.cotas || []).forEach(c => rows.push(['  cota', '', c.grupo, c.cota, c.cliente || '', c.valor, '', '']));
  // REPASSES A SUPERVISORES (laranja 0,2%)
  (f.repassesSupervisor || []).forEach(rp => {
    rows.push(['Repasse supervisor (0,2%)', rp.nome + (rp.time ? ' · ' + rp.time : ''), '', '', '', rp.cheio, rp.imposto, rp.liquido]);
    (rp.cotas || []).forEach(c => rows.push(['  cota', '', c.grupo, c.cota, c.cliente || '', c.valor, '', '']));
  });
  // REPASSES A OUTROS GESTORES ROXOS (0,5% integral)
  (f.repassesRoxo || []).forEach(rp => {
    rows.push(['Repasse gestor (0,5%)', rp.nome + (rp.time ? ' · ' + rp.time : ''), '', '', '', rp.cheio, rp.imposto, rp.liquido]);
    (rp.cotas || []).forEach(c => rows.push(['  cota', '', c.grupo, c.cota, c.cliente || '', c.valor, '', '']));
  });
  // CONSULTORES (autorizado)
  (f.porConsultor || []).forEach(pc => {
    rows.push(['Consultor', pc.nome, '', '', '', pc.cheio, pc.imposto, pc.liquido]);
    (pc.cotas || []).forEach(c => rows.push(['  cota', '', c.grupo, c.cota, c.cliente || '', c.valor, '', '']));
  });
  // NÃO BATERAM
  if ((f.naoBateram || []).length) {
    rows.push(['Não bateram', '(sem consultor na carteira)', '', '', '', f.naoBateuTotal || 0, '', '']);
    (f.naoBateram || []).forEach(n => rows.push(['  cota', '', n.grupo, n.cota, n.cliente || '', n.valor, '', '']));
  }
  // TOTAIS
  rows.push(['', '', '', '', '', '', '', '']);
  rows.push(['TOTAL', 'Cheio / Imposto / A repassar', '', '', '', f.totalCheio, f.totalImposto, f.totalRepasse != null ? f.totalRepasse : f.totalLiquido]);
  rows.push(['TOTAL', 'Sua retenção líquida', '', '', '', '', '', g.liquido || 0]);
  const buf = xlsx.build(headers, rows, { moneyCols: [5, 6, 7], sheetName: 'Repasse ' + (f.competencia || ''), widths: [20, 26, 10, 10, 26, 14, 12, 14] });
  return { ok: true, buffer: buf, filename: 'comissoes-' + (f.competencia || 'repasse') + '.xlsx' };
}
...
// ===== BACKUP: dump completo e legível de todas as cotas (vendas + carteira) =====
// Usado pelo endpoint protegido por token /api/backup/vendas para a rotina diária de backup no Drive.
function backupDados() {
  const vendas = (demo.vendas || []).map(v => ({
    data: v.data || '',
    cliente: (v.cliente && v.cliente.nome) || '',
    cpf: (v.cliente && v.cliente.cpf) || '',
    ...
    grupo: v.grupo || '',
    cota: v.cota || '',
    tamanhoGrupo: +v.tamanhoGrupo || 0,
    valor: +v.valor || 0,
    status: v.status || '',
    situacao: v.situacao || 'ativo',
    carteira: !!v.carteira,
  }));
  return { ok: true, geradoEm: new Date().toISOString(), total: vendas.length, vendas };
}

module.exports = { homeConsultor, criarSolicitacao, registrarCheckin, finalizarCheckin, atualizarAtividade, painelGestor, agendaGestor, decidirSolicitacao, cancelarSolicitacao, criarAtividade, cancelarAtividade, listarAvisos, marcarAvisoLido, excluirAviso, perfilConsultor, controleGestor, rankingGestor, definirMeta, listarChat, enviarChat, chatNaoLidas, listarRotinas, salvarRotinas, destinatarios, disponibilidade, excluirSolicitacao, listarGestores, listarArquivos, salvarArquivo, excluirArquivo, arquivoParaDownload, listarTimes, criarTime, excluirTime, cancelarSolicitacaoGestor, configContratacao, listarContratacao, marcarContratacao, listarVendas, exportarVendasXlsx, sortearProximidade, gruposParaSorteio, clientesPorConsultor, modeloClientesXlsx, importarClientesXlsx, criarVenda, editarVenda, excluirVenda, dashboardVendas, listarVendasLog, negociacoesTodasEquipes, listarClientes, solicitarCadastro, listarCadastros, decidirCadastro, solicitarReuniaoGestor, muralDados, muralSalvar, comImpostosGet, comImpostosSalvar, comImportar, comListarFechamentos, comFechamento, comExcluirFechamento, comExportarXlsx, backupDados, DEMO };
