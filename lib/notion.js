...
async function comImportar(session, { arquivo, competencia } = {}) {
  ...
  // índice da carteira (mesmo escopo da listagem de vendas do gestor; master = tudo)
  const base = await listarVendas(session, {});
  const idx = {};
  (base.vendas || []).forEach(v => { const key = chaveGC(v.grupo, v.cota); if (!idx[key]) idx[key] = v; });
  // separa AUTORIZADO (por consultor) e LICENCIADO (gestor)
  const porConsMap = {}; const naoBateram = []; let totalCheio = 0;
  let gestorCheio = 0; const gestorCotas = [];
  Object.values(porCota).forEach(rc => {
    const aut = Math.round(rc.aut * 100) / 100, lic = Math.round(rc.lic * 100) / 100;
    totalCheio += aut + lic;
    if (lic > 0) { gestorCheio += lic; gestorCotas.push({ grupo: rc.grupo, cota: rc.cota, cliente: rc.cliente, valor: lic }); }
    if (aut > 0) {
      const v = idx[chaveGC(rc.grupo, rc.cota)];
      if (!v) { naoBateram.push({ grupo: rc.grupo, cota: rc.cota, cliente: rc.cliente, valor: aut }); return; }
  ...
  gestorCheio = Math.round(gestorCheio * 100) / 100;
  const impostos = comImpostosDe(session);
  const porConsultor = Object.values(porConsMap).map(pc => {
    const cheio = Math.round(pc.cheio * 100) / 100;
    const imposto = comCalcImposto(impostos, cheio, totalCheio);
    return { email: pc.email, nome: pc.nome, cheio, imposto, liquido: Math.round((cheio - imposto) * 100) / 100, qtdCotas: pc.cotas.length, cotas: pc.cotas.sort((a, b) => b.valor - a.valor) };
  }).sort((a, b) => b.liquido - a.liquido);
  const gImp = comCalcImposto(impostos, gestorCheio, totalCheio);
  const gestor = { nome: session.nome || session.email, cheio: gestorCheio, imposto: gImp, liquido: Math.round((gestorCheio - gImp) * 100) / 100, qtdCotas: gestorCotas.length, cotas: gestorCotas.sort((a, b) => b.valor - a.valor) };
  const totalImposto = Math.round((gestor.imposto + porConsultor.reduce((s, x) => s + x.imposto, 0)) * 100) / 100;
  const totalRepasse = Math.round(porConsultor.reduce((s, x) => s + x.liquido, 0) * 100) / 100; // líquido dos consultores (autorizado)
  const naoBateuTotal = Math.round(naoBateram.reduce((s, x) => s + x.valor, 0) * 100) / 100;
  const comp = /^\d{4}-\d{2}$/.test(String(competencia || '')) ? competencia : mesAtualBR();
  const fech = {
    id: 'f' + Date.now() + Math.random().toString(36).slice(2, 5),
    gestorEmail: String(session.email || '').toLowerCase(), gestorNome: session.nome || session.email, time: session.time || '',
    ...
    gestor, porConsultor, naoBateram: naoBateram.sort((a, b) => b.valor - a.valor), naoBateuTotal,
  };
...
function comExportarXlsx(session, { id } = {}) {
  ...
  // GESTOR (licenciado)
  rows.push(['Gestor (licenciado)', g.nome || f.gestorNome, '', '', '', g.cheio || 0, g.imposto || 0, g.liquido || 0]);
  ...
  rows.push(['TOTAL', 'Seu líquido (licenciado)', '', '', '', '', '', g.liquido || 0]);
