...
async function criarAtividade(session, { consultor, titulo, tipo, prazo, recorrente }) {
  if (!titulo || !consultor) return { erro: 'Preencha consultor e atividade' };
  if (DEMO) {
    demo.atividades.unshift({ id: 'a' + Date.now(), titulo, consultor, tipo: tipo || 'Outro', prazo: prazo || hoje(), status: 'A fazer', recorrente: !!recorrente });
    novoAviso(consultor, 'Nova atividade', `Seu gestor criou: "${titulo}" (${tipo || 'Outro'})${recorrente ? ' 🔁 semanal' : ''}${prazo ? ' — prazo ' + prazo.slice(8, 10) + '/' + prazo.slice(5, 7) : ''}.`);
    return { ok: true };
  }
  await notionFetch('pages', {
