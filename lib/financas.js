...
function acumuladoMetas(d) {
  const soma = {};
  Object.values(d.meses || {}).forEach(m => (m.despesa || []).forEach(l => { if (l.meta) soma[l.meta] = (soma[l.meta] || 0) + n2(l.valor); }));
  return (d.metas || []).map(m => ({ nome: m.nome, alvo: n2(m.alvo), guardado: n2(m.guardado), img: m.img || '', vinculado: n2(soma[m.nome] || 0), guardadoTotal: n2((+m.guardado || 0) + (soma[m.nome] || 0)) }));
}
function totais(m) {
...
function getMes(mes) {
  ...
  return { mes, renda: m.renda || [], despesa: m.despesa || [], metas: acumuladoMetas(d), resumo: totais(m) };
}
function salvarMes(mes, renda, despesa) {
  ...
  save(d);
  return { ok: true, resumo: totais(d.meses[mes]), metas: acumuladoMetas(d) };
}
function salvarMetas(metas) {
  const d = db();
  d.metas = (metas || []).filter(x => x.nome && String(x.nome).trim()).map(x => ({ nome: String(x.nome).slice(0, 60), alvo: n2(x.alvo), guardado: n2(x.guardado), img: String(x.img || '').slice(0, 1500000) }));
  save(d);
  return { ok: true, metas: d.metas };
}
function garantirMes(data, key) {
  if (data.meses[key]) return data.meses[key];
  const keys = Object.keys(data.meses).filter(k => k < key).sort();
  const ant = keys.length ? data.meses[keys[keys.length - 1]] : null;
  const m = ant ? JSON.parse(JSON.stringify(ant)) : { renda: [], despesa: [] };
  data.meses[key] = m; return m;
}
// Repete uma despesa parcelada (obs "N/T") nos meses seguintes, incrementando a parcela até a última
function propagarParcela(mesInicial, d) {
  const mm = String(d.obs || '').match(/^(\d+)\s*\/\s*(\d+)$/);
  ...
  while (n < t && count < 240) {
    ...
    const mes = garantirMes(data, key);
    const ex = (mes.despesa || []).find(x => x.item === d.item);
    if (ex) { ex.obs = n + '/' + t; ex.valor = n2(d.valor); ex.meta = d.meta || ''; ex.investimento = !!d.investimento; }
    else { mes.despesa.push({ item: d.item, obs: n + '/' + t, valor: n2(d.valor), pago: false, meta: d.meta || '', investimento: !!d.investimento }); }
    count++;
  }
  save(data);
...
function resumoAno(ano) {
  ...
  return { ano, receitas: n2(receitas), despesas: n2(despesas), saldo: n2(receitas - despesas), meses, metas: acumuladoMetas(d) };
}

function investimentos(ano) {
  const d = db();
  ano = ano || String(new Date().getFullYear());
  const meses = []; let total = 0; const porItem = {}; const lancs = [];
  for (let i = 1; i <= 12; i++) {
    const k = ano + '-' + String(i).padStart(2, '0');
    const m = d.meses[k]; let mt = 0;
    if (m) (m.despesa || []).forEach(l => { if (l.investimento) { mt += n2(l.valor); const nm = l.item || 'Outros'; porItem[nm] = (porItem[nm] || 0) + n2(l.valor); lancs.push({ mes: k, item: nm, valor: n2(l.valor), meta: l.meta || '' }); } });
    meses.push({ mes: k, total: n2(mt) }); total += mt;
  }
  const itens = Object.entries(porItem).map(([nome, valor]) => ({ nome, valor: n2(valor) })).sort((a, b) => b.valor - a.valor);
  return { ano, total: n2(total), meses, itens, lancamentos: lancs.reverse() };
}

module.exports = { login, sessao, clearCookie, trocarSenha, getMes, salvarMes, salvarMetas, resumoAno, investimentos, propagarParcela };
